...
RAYGUIAPI GuiState GuiGetLocalState(void);                      // Get gui state (local state)
...
RAYGUIAPI int GetTextWidth(const char *text);
...
RAYGUIAPI int GuiDropdownBox(Rectangle bounds, const char *text, int *active, bool editMode, char* item=nullptr); // Dropdown Box control, returns selected item
...
static GuiState guiLocalState = STATE_NORMAL;   // Gui local state, state of current control
...
// Get gui state (local state)
GuiState GuiGetLocalState(void) { return guiLocalState; }
...
// Label control
int GuiLabelTip(Rectangle bounds, const char *text)
{
    int result = 0;
    GuiState state = guiState;
    ...
    // Draw control
    //--------------------------------------------------------------------
    int textcolour = GuiGetStyle(DEFAULT, TEXT_COLOR_PRESSED);
    GuiDrawText(text, GetTextBounds(LABEL, bounds), GuiGetStyle(LABEL, TEXT_ALIGNMENT), GetColor(textcolour));
    //--------------------------------------------------------------------
...
int GuiDropdownBox(Rectangle bounds, const char *text, int *active, bool editMode, char* item)
{
    int result = 0;
    GuiState state = guiState;
    ...
    *active = itemSelected;
    if(item) strcpy(item, items[itemSelected]);
    guiLocalState = state;
...
int GuiTextBox(Rectangle bounds, char *text, int bufferSize, bool editMode)
{
        ...
        #define RAYGUI_TEXTBOX_AUTO_CURSOR_COOLDOWN  500        // Frames to wait for autocursor movement
    #endif
    #if !defined(RAYGUI_TEXTBOX_AUTO_CURSOR_DELAY)
        #define RAYGUI_TEXTBOX_AUTO_CURSOR_DELAY     5        // Frames delay for autocursor movement
    #endif
...
{ 
    if (!guiLocked && guiTooltip && (guiTooltipPtr != NULL) && !guiSliderDragging)
    {
        ...
        controlRec.x += (controlRec.width - (textSize.x + 16)) / 2;
        if(controlRec.x < 0) controlRec.x = 0.0f;
        float edge = controlRec.x + textSize.x + 16 - GetScreenWidth();
        if (edge > 0) controlRec.x -= edge;

        float height = float(GuiGetStyle(DEFAULT, TEXT_SIZE));
        int backColour = GuiGetStyle(DEFAULT, BACKGROUND_COLOR);
        GuiSetStyle(DEFAULT, BACKGROUND_COLOR, GuiGetStyle(DEFAULT, BASE_COLOR_PRESSED));
        GuiPanel(RAYGUI_CLITERAL(Rectangle){ controlRec.x, controlRec.y + controlRec.height*1.3f, textSize.x + 16, height*3 }, NULL);
        GuiSetStyle(DEFAULT, BACKGROUND_COLOR, backColour);

        int textPadding = GuiGetStyle(LABEL, TEXT_PADDING);
        int textAlignment = GuiGetStyle(LABEL, TEXT_ALIGNMENT);
        GuiSetStyle(LABEL, TEXT_PADDING, 0);
        GuiSetStyle(LABEL, TEXT_ALIGNMENT, TEXT_ALIGN_CENTER);
        GuiLabelTip(RAYGUI_CLITERAL(Rectangle){ controlRec.x, controlRec.y + controlRec.height*1.3f, textSize.x + 16, height*3 }, guiTooltipPtr);
...
static Rectangle GetTextBounds(int control, Rectangle bounds)
{
    ...
    textBounds.width = bounds.width - 2*GuiGetStyle(control, BORDER_WIDTH) - 2*GuiGetStyle(control, TEXT_PADDING) - 4;
