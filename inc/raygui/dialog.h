...
typedef struct {
    ...
    char filterExt[256];
    char dirPathTextCopy[DLG_STR_LEN];
    char fileNameTextCopy[DLG_STR_LEN];
    char dialogTitle[DLG_STR_LEN];
    char selectText[DLG_STR_LEN] = "Select";
...
void GuiWindowFileDialog(GuiWindowFileDialogState *state)
{
    if (state->windowActive)
    {
        ...
        //----------------------------------------------------------------------------------------
        char title[DLG_STR_LEN + 16] = "#198# ";
        strcat(title, state->dialogTitle);
