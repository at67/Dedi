...
- Copy the contents of Dedi.zip into this newly created folder, eg: C:\Dedi<br/>
 ...
 - If you are installing Dedi and the Aska Dedicated Server on a different PC/Laptop to your PC/Laptop that the main Aska game is installed on, then you need to do the following:<br/>
	- You need to login to the same steam account that your purchased the main game game on.<br/>
	- Download/install the Aska Dedicated Server.<br/>
	- Install Dedi to this PC/Laptop, NOT to the same folder as the Aska Dedicated Server.<br/>
	- From then on when you launch the Aska dedicated server, (through Dedi), it will login anonymously into SteamCmd using your authorisation token saved in your "server properties.txt" file to launch the actual server.<br/>
	- You don't need multiple steam accounts and you don't need to be logged into Steam to launch the Aska Dedicated Server, (you only need to login to Steam once to download/install it).<br/>
...
<br/>
If you are trying to run Dedi from a VM, it requires at a minimum of OpenGL3.3, so you must enable "3D acceleration", (or similar), in your VM settings and then pray.<br/>
